...
export default function QuizPage() {
    ...
    const chosenDifficulty = searchParams.get("difficulty") || "easy";
    const user = getUser();
    ...
    const [timedOut, setTimedOut] = useState(false);  // true when time expired
    const timerRef = useRef(null);
    const sessionRef = useRef(null); // stable ref so callbacks always see latest sessionId
    const questionNumRef = useRef(0); // stable ref for questionNum inside timer callbacks
    ...
    useEffect(() => {
        ...
        async function init() {
            try {
                const data = await startQuiz(user.id, topic, chosenDifficulty);
                if (data.error) {
    ...
    async function fetchNextQuestionWith(sid) {
        ...
        try {
            ...
            setQuestion(q);
            setDifficulty(q.difficulty || chosenDifficulty);
            setQuestionNum((n) => {
                const next = n + 1;
                questionNumRef.current = next;
                return next;
            });
            setTimedOut(false);
    ...
    function handleTimeout() {
        ...
        setTimedOut(true);
        setSelected("__timeout__");  // blocks further clicks
        // Don't mark any wrong option — just reveal the correct answer
        ...
        if (questionNumRef.current >= TOTAL_QUESTIONS) {
            ...
            setTimeout(() => fetchNextQuestion(), 2500);
        }
    }
    ...
    async function handleAnswer(option) {
        ...
        try {
            ...
            setResult(res);
            // Difficulty stays fixed — do NOT update from server response
            if (res.is_correct) {
                setScore((s) => s + 10);
            }
        } catch {
    ...
    return (
        <div>
            ...
            <div className="quiz-page">
                    ...
                    <div className="quiz-options">
                        {question.options.map((opt, i) => {
                            ...
                            if (selected !== null) {
                                ...
                                // Only mark wrong if user actually selected it (not a timeout)
                                else if (opt === selected && !timedOut) cls += " wrong";
                            }
                            return (
                ...
                {/* Timeout notice */}
                {timedOut && (
                    <div className="quiz-timeout-msg">
